...
## Flutter Mobile (parity pass)

Mobile mirrors web visual language via `google_fonts` + shared tokens in `mobile/lib/core/theme/app_theme.dart`.

### Typography (Flutter)

| Role | Font | Weight | Size |
|------|------|--------|------|
| Display / H1 | **Outfit** | 700 | 28px |
| H2 / Section | **Outfit** | 600 | 20px |
| Body | **DM Sans** | 400 | 16px |
| Label / Caption | **DM Sans** | 500 | 12–13px |
| Price / Emphasis | **Outfit** | 700 | 18px |

### Sport badge colors (Flutter)

| Sport | Background | Text |
|-------|------------|------|
| badminton | `#EDE9FE` | `#6D28D9` |
| mini_football | `#DBEAFE` | `#1D4ED8` |
| tennis | `#D1FAE5` | `#047857` |

### Court images (bundled assets)

| Asset | Sport |
|-------|-------|
| `assets/images/badminton.jpg` | badminton |
| `assets/images/mini_football.jpg` | mini_football |
| `assets/images/tennis.jpg` | tennis |
| `assets/images/placeholder.png` | fallback |

Resolver: asset by `sport_type` → network `image_url` → placeholder.

### Mobile components

- **CourtCard** — 16:9 image, sport badge overlay top-left, title, capacity icon, price accent, full-width "Đặt sân" CTA
- **FilterChipRow** — All / Cầu lông / Mini football / Tennis (horizontal scroll)
- **StatusPill** — pending (amber), confirmed (green), cancelled (red)
- **SkeletonCourtCard** — pulse placeholder while loading
- **AppPageHeader** — title + subtitle, no generic AppBar-only screens

### Mobile screen notes

| Screen | Layout |
|--------|--------|
| Login | Centered card max ~400px; logo + tagline; demo hint in muted box |
| Courts | Page header + filter chips + court card list + pull-to-refresh |
| Book court | Hero image top; slot chips (available/selected/disabled); sticky footer with total + CTA ≥48px |
| My bookings | Tabs upcoming/past; row with 56px thumbnail + status pill; empty state + CTA |
| Shell | Bottom nav: Sân · Lịch đặt · Tài khoản; logout tại tab Tài khoản |
| Account | App intro card (logo, tagline, mô tả, sport badges) + user info card (tên, email, vai trò) + đăng xuất; pull-to-refresh `/auth/me` |
| Web `/profile` | Cùng intro card + `AppLogo` như mobile tab Tài khoản |
